...
int GenID = 0;
    ...
    {
       // int id_parent=0;
       // if (id_parent>NbIndividus)id_parent=0;
        ...
        foreach (Individu *ind, liste_individu)
        {
            ...
            if (ind->skill()<seuil_a_atteindre){
                mutated=ind->mutate();}
            //si il y a eu mutation: on récupère le numéro de la generation
            if(mutated){
                ind->generID = GenID;
                liste_individu.replace(ind->id(), new Individu(ind));
            }
...
void Population::evolutionnaryProcess()
{
    ...
    {
        cout<<endl<<"***son ratio est trop eleve pour beneficier d'une mutation***"<<endl;
    }
}
