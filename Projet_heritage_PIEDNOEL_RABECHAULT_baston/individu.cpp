...
bool Individu::mutate()//return faux si la mutation a échouée
{
 ...
 if (val<genome[index]||(val2*Parameters::mutationRate)<TAILLE_GENOME||index==0)return false;
...
//fonction pour dupliquer un individu
Individu::Individu(Individu *papa)

{
    victoire = papa->getVictoire();
    defaite = papa->getDefaite();
    ratio = papa->getRatio();
    generID = papa->generID;
    generNB = papa->generNB;
